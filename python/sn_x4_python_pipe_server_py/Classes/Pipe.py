...
class Pipe_Server(Pipe):
    ...
    def __init__(self, pipe_name, buffer_size = None, verbose = False):
        ...
        # WORKAROUND: Use NULL DACL for maximum compatibility
        # This allows any process to connect to the pipe
        # Some Windows configurations have security context mismatches between
        # the pipe server and X4's winpipe DLL, causing "access denied" errors
        # even when running as the same user.
        # See: https://docs.microsoft.com/en-us/windows/win32/secauthz/null-dacls-and-empty-dacls
        sec_attr = None
        if self.verbose:
            print('Using default security (NULL DACL) for pipe access')
